...
const WaveVisualization = ({ isPlaying }: WaveVisualizationProps) => {
    ...
    return (
        <div className="xl:flex hidden items-center justify-center gap-2 h-10">
            {animatedBars.map((height, index) => (
                <div
                    key={index}
                    className={`bg-gradient-to-t from-[#9578ec] to-[#7B57E4]  rounded-full transition-all duration-200 ${isPlaying ? "opacity-100 shadow-lg " : "opacity-40"
                        }`}
                    style={{
                        width: "4px",
                        height: `${height}px`,
